...
**Always use the helpers in `app/db/migration_helpers.py` for Postgres enums.** SQLAlchemy's `sa.Enum` auto-creates the `TYPE` on first use, which breaks re-runs, shared enums across tables, and downgrade/upgrade cycles. The helpers emit idempotent raw SQL:
...
The `create_type=False` on the column is the critical half — without it SQLAlchemy still tries to auto-create the type and blows up. To add a value later, use `add_enum_value("user_role", "guest")`.

Autogenerated migrations frequently include false-positive index drops/creates from unrelated tables. **Always review the diff by hand.** Test every migration against both a clean DB and one at the current production head before merging.
